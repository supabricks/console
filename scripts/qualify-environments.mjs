...
import { qualifyEnvironments } from "./environments.mjs";
...
const append = report.checks.push.bind(report.checks);
...
async function cli(...command) {
  return cliAt(project, ...command);
}
async function cliAt(at, ...command) {
  try {
    const result = await exec(
      resolve(args["--binary"]),
      [...command, "--project", at, "--data-dir", data],
...
try {
  ...
  await qualifyEnvironments(page, {
    project,
    root,
    data,
    screenshot: resolve(args["--report"]).replace(/\.json$/, ".png"),
    cli,
    cliAt,
    browser,
  ...
  await cli("down")
    ...
    .catch(() => {
      report.status = "failed";
      report.errors.push("Fixture shutdown failed");
      process.exitCode = 1;
    });
  if (stopped && report.status === "passed")
